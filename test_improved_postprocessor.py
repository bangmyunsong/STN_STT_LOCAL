...
def test_improved_postprocessor():
    ...
    try:
        ...
        # 개선된 프롬프트와 후처리 로직으로 ERP 데이터 추출
        print("🤖 개선된 프롬프트 + 후처리 로직으로 ERP 데이터 추출 중...")
        result = extractor.extract_erp_data(session100_text, filename="session100_improved_test")
        
        print("🔍 개선된 후처리 로직 추출 결과:")
        print("="*50)
...
if __name__ == "__main__":
    success = test_improved_postprocessor()
    if success:
        print("\n✅ 개선된 후처리 로직 테스트 성공!")
    else:
        print("\n💥 개선된 후처리 로직 테스트 실패!")
