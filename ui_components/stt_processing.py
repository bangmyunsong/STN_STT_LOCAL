...
def show_stt_processing():
    ...
    with tab1:
        ...
        if success and (audio_data.get("files") or audio_data.get("daily_files")):
            ...
            with col1:
                st.write("**처리할 파일 선택**")
                available_options = [f['display_name'] for f in available_for_selection]
                
                selected_available = st.multiselect(
                    "파일을 선택하세요:",
                    options=available_options,
                    key="available_files_select",
            ...
            with col2:
                ...
                if st.button("➡️", key="move_to_target", 
                    ...
                    for selected_name in selected_available:
                ...
                if st.button("⬅️", key="move_from_target",
                           ...
                           disabled=not st.session_state.get('target_files_select', [])):
                    # 선택된 파일들을 STT 처리 대상에서 제거
                    selected_target = st.session_state.get('target_files_select', [])
                    st.session_state.stt_target_files = [
                        f for f in st.session_state.stt_target_files 
                        if f['display_name'] not in selected_target
                    ]
                    st.rerun()
            
            with col3:
                st.write("**STT 처리 대상**")
                target_options = [f['display_name'] for f in st.session_state.stt_target_files]
                
                selected_target = st.multiselect(
                    "처리 대상 파일:",
                    options=target_options,
                    key="target_files_select",
                    label_visibility="collapsed"
                )
            ...
            if st.session_state.stt_target_files:
                ...
                with col3:
                    save_to_db = st.checkbox("DB 저장", value=True, key="stt_save_db")
                with col4:
                ...
                with col2:
                    if st.button("🚀 STT 처리 시작", key="start_stt_processing", type="primary"):
                        ...
                        process_bulk_files(
                            ...
                            process_audio_file_from_directory, register_erp_sample, save_to_db
                        )
